...
function moveM(state: CubeStateData): CubeStateData {
  const s = cloneState(state);
  // U[1,4,7] -> F[1,4,7] -> D[1,4,7] -> B[7,4,1]
...
export type MoveType = 'U' | "U'" | 'D' | "D'" | 'F' | "F'" | 'B' | "B'" | 'L' | "L'" | 'R' | "R'" | 'M' | "M'" | 'E' | "E'" | 'S' | "S'";
...
const ALL_MOVES: MoveType[] = ['U', "U'", 'D', "D'", 'F', "F'", 'B', "B'", 'L', "L'", 'R', "R'"];

export function scrambleCube(state: CubeStateData, moves: number = 20): { state: CubeStateData; sequence: MoveType[] } {
